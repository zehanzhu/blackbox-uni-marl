...
class PolicyRoutingLLMClient:
    """Route generation calls to per-policy LLM server clients."""

    def __init__(self, policy_clients: dict[str, LLMServerClient]):
        if not policy_clients:
            raise ValueError("policy_clients must not be empty")
        self.policy_clients = dict(policy_clients)
        ...
        **kwargs: Any,
        ...
        if policy_name is None:
            if len(self.policy_clients) != 1:
                raise ValueError("policy_name is required when multiple policy clients are configured")
            policy_name = next(iter(self.policy_clients))
        client = self.policy_clients.get(policy_name)
        if client is None:
...
class GatewayServingRuntime:
    ...
    async def create_multi_agent_rollout(self, rollout_id: str, **kwargs):
        gateway_manager = self._require_session_runtime()
        return await gateway_manager.create_multi_agent_rollout(rollout_id=rollout_id, **kwargs)

    async def finalize_multi_agent_rollout(self, rollout_id: str):
        gateway_manager = self._require_session_runtime()
        return await gateway_manager.finalize_multi_agent_rollout(rollout_id=rollout_id)

    async def complete_multi_agent_rollout(self, rollout_id: str, reward_info: dict[str, Any] | None = None) -> None:
        gateway_manager = self._require_session_runtime()
        await gateway_manager.complete_multi_agent_rollout(rollout_id=rollout_id, reward_info=reward_info)

    async def abort_multi_agent_rollout(self, rollout_id: str) -> None:
        gateway_manager = self._require_session_runtime()
        await gateway_manager.abort_multi_agent_rollout(rollout_id=rollout_id)
    ...
    async def wait_for_multi_agent_rollout_completion(
        ...
        gateway_manager = self._require_session_runtime()
        await gateway_manager.wait_for_multi_agent_rollout_completion(rollout_id=rollout_id, timeout=timeout)
