...
class GatewayManager:
    ...
    def __init__(self, gateways: list):
        ...
        self._rollout_to_gateway_index: dict[str, int] = {}
    ...
    def _get_rollout_gateway(self, rollout_id: str):
        gateway_index = self._rollout_to_gateway_index.get(rollout_id)
        if gateway_index is None:
    ...
    async def create_multi_agent_rollout(self, rollout_id: str, **kwargs):
        gateway_index = self._select_gateway_index()
        gateway = self.gateways[gateway_index]
        handle = await _await_object_ref(
            gateway.create_multi_agent_rollout.remote(rollout_id=rollout_id, **kwargs)
        )
        self._rollout_to_gateway_index[rollout_id] = gateway_index
        # Count a multi-agent rollout as one scheduling unit; role sessions remain actor-local state.
        self.active_sessions_per_gateway[gateway_index] += 1
        return handle

    async def finalize_multi_agent_rollout(self, rollout_id: str):
        gateway, gateway_index = self._get_rollout_gateway(rollout_id)
        result = await _await_object_ref(gateway.finalize_multi_agent_rollout.remote(rollout_id=rollout_id))
        self._rollout_to_gateway_index.pop(rollout_id, None)
        self.active_sessions_per_gateway[gateway_index] -= 1
        return result
    ...
    async def complete_multi_agent_rollout(
        ...
        gateway, _ = self._get_rollout_gateway(rollout_id)
        await _await_object_ref(
            gateway.complete_multi_agent_rollout.remote(rollout_id=rollout_id, reward_info=reward_info)
        )

    async def abort_multi_agent_rollout(self, rollout_id: str) -> None:
        gateway, gateway_index = self._get_rollout_gateway(rollout_id)
        await _await_object_ref(gateway.abort_multi_agent_rollout.remote(rollout_id=rollout_id))
        self._rollout_to_gateway_index.pop(rollout_id, None)
        self.active_sessions_per_gateway[gateway_index] = max(
            0,
            self.active_sessions_per_gateway[gateway_index] - 1,
        )

    async def wait_for_multi_agent_rollout_completion(
        self,
        rollout_id: str,
        ...
        gateway, _ = self._get_rollout_gateway(rollout_id)
        await _await_object_ref(
            gateway.wait_for_multi_agent_rollout_completion.remote(rollout_id=rollout_id, timeout=timeout)
        )
